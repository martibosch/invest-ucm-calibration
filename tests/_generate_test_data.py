...
def dump_rasters(raster_filepaths, low, high, nodata, data_mask, dtype, meta):
    for raster_filepath in raster_filepaths:
        with rio.open(raster_filepath, 'w', dtype=dtype, nodata=nodata,
                      **meta) as dst:
            dst_arr = rn.uniform(low, high,
                                 meta['height'] * meta['width']).reshape(
                                     meta['height'],
                                     meta['width']).astype(dtype)
...
with rio.open(lulc_raster_filepath, 'w', dtype=lulc_dtype, nodata=lulc_nodata,
              **meta) as dst:
    dst_arr = np.digitize(
        rn.random(num_pixels).reshape(shape),
        np.linspace(0, 1, 4)).astype(lulc_dtype)
